...
async function database(t) {
  const file = path.join(directory, `accessi-schema-${process.pid}-${++counter}.fdb`);
  const databaseOptions = { host: '127.0.0.1', port, database: file, user: 'SYSDBA', password: process.env.ACCESSI_TEST_DB_PASSWORD || 'masterkey', blobAsText: true };
...
integration('generic SQL snapshot and standalone read-only check match the schema contract', async t => {
  ...
  for (const sql of ddl.replace(/--[^\n]*/g, '').split(';').map(s => s.trim()).filter(Boolean)) {
    if (!sql.startsWith('SET SQL DIALECT')) await exec(o, sql);
  }
  for (const sql of rest.split('SET TERM ; ^')[0].split('^').map(s => s.trim()).filter(Boolean)) await exec(o, sql);
