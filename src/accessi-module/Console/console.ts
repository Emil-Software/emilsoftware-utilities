...
async function showRoles(): Promise<void> {
  ...
  render(`<div class="toolbar"><button id="new-role">Nuovo ruolo</button></div><h2>Ruoli e grant</h2><p class="form-help">Ruoli con i menu e il livello assegnato.</p><div class="role-overview-list">${roles.map((role) => roleOverview(role, groups)).join('') || '<p class="muted">Nessun ruolo configurato.</p>'}</div>`);
...
async function showMenus(): Promise<void> {
  ...
  render(`<h2>Menu e gruppi</h2><p class="form-help">Attiva o disattiva gruppi e menu.</p><div class="menu-group-list">${groups.map(menuGroupTable).join('')}</div>`);
